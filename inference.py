...
parser.add_argument('--analogy', default=False, action='store_true')
...
cls_train_steps = [
    ...
    {'dataset': 'APY', 'num_shots': 1, 'generalized': True, 'cls_train_steps': 78},
    {'dataset': 'APY', 'num_shots': 1, 'generalized': False, 'cls_train_steps': 78}
]
...
hyperparameters['cls_train_steps'] = [
    x['cls_train_steps'] for x in cls_train_steps
    if all([hyperparameters['dataset']==x['dataset'], hyperparameters['num_shots']==x['num_shots'], hyperparameters['generalized']==x['generalized']])
][0]
...
if args.train_mode=='standard':
    saved_state = torch.load('models/CADA_trained_'+args.dataset+'_'+args.mode+'_'+str(args.discrete)+'_'+str(args.run)+'_'+str(args.fraction)+'.pth.tar')
else:
    saved_state = torch.load('models/CADA_trained_'+args.dataset+'_'+args.mode+'_'+args.train_mode+'_'+str(args.discrete)+'_'+str(args.run)+'_'+str(args.fraction)+'.pth.tar')
